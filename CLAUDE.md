...
Phases 1–5 are done: all 56 sentence types, the Flow-based IO and correlating operators, AIS and u-blox. What remains is phase 6, the cutover — deleting `src/jvmMain/java`, rewriting `:examples` in Kotlin, and swapping `javadocJvm` for Dokka.
...
- **The Kotlin port disagrees with the Java tree in places, deliberately.** Where a Kotlin type's behaviour differs from its Java predecessor the reason is on the declaration, usually because the Java one was wrong: AIS types 4, 18 and 27 read the position-accuracy bit one place early, type 27's navigational status range was reversed, type 9's flags were off by one and its speed scaled by ten, `isDteReady` returned the bit uninverted, `NavStatus` read `V` as valid, and `PositionProvider` required a GGA or GLL in every cycle. The Java suite pins some of these wrong answers, so a Kotlin test contradicting a Java test is expected — check the comment before "fixing" it.
