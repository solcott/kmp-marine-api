...
- **Everything except the Apple targets cross-compiles from any host.** That is what lets the release workflow publish all fourteen publications from one macOS runner, and it is why the macOS `all-targets` job is the slow one. Only Apple is host-locked.
