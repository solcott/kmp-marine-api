...
- **`:examples-android` names `kotlin-test-junit` explicitly, and must.** There is no `useJUnit()` hook in an AGP unit test to make `kotlin-test` resolve to a framework variant, so plain `kotlin-test` resolves to the frameworkless one and `kotlin.test.Test` does not exist. The failure is at least a compile error rather than :marine-api's silent zero-tests-and-green.
